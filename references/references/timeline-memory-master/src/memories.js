...
const infoToast = (text) => {
  if (!commandArgs?.quiet) toastr.info(text, "Timeline Memory");
};
const doneToast = (text) => {
  if (!commandArgs?.quiet) toastr.success(text, "Timeline Memory");
};
const oopsToast = (text) => {
  if (!commandArgs?.quiet) toastr.warning(text, "Timeline Memory");
};
const errorToast = (text) => {
  if (!commandArgs?.quiet) toastr.error(text, "Timeline Memory");
};
    ...
    if (profile.preset) {
      ...
      switch (profile.api) {
        ...
        default:
          // Generic fallback
          maxTokens =
            presetSettings.max_tokens || presetSettings.max_length || presetSettings.genamt;
...
function buildOverridePayload(profileId, maxTokens) {
  try {
    ...
    if (profile && profile.api === "openai" && profile.model) {
      ...
      const needsSpecialParams =
        profile.model.startsWith("o1") ||
...
export function updateTimelineInjection() {
  ...
  const timelineStr =
    typeof timelineContext === "string"
      ? timelineContext
      : JSON.stringify(timelineContext, null, 2);
  const timelineResponsesStr =
    typeof timelineResponsesContext === "string"
      ? timelineResponsesContext
      : JSON.stringify(timelineResponsesContext, null, 2);
...
export function migrateTimelineData() {
  ...
  if (!timelineData || timelineData.length === 0) {
    return {
      migrated: 0,
      hadTimestamps: false,
      convertedFromPlaintext: false,
      convertedSceneToChapter: false,
    };
  }
  ...
  if (context.chatMetadata?.timeline && typeof context.chatMetadata.timeline === "string") {
    ...
    const chapterRegex =
      /(?:Scene|Chapter)\s+(\d+)\s+\(Messages\s+(\d+)-(\d+)\):\s+(.+?)(?=\n\n(?:Scene|Chapter)\s+\d+|$)/gs;
    const newTimeline = [];
    let match;
...
export async function getChapterHistory(chapterNumber) {
  ...
  const processedMessages = await Promise.all(
    chapterMessages.map(async (message, index) => {
...
function uniqueSortedChapters(chapters) {
  return Array.from(new Set(chapters.filter((num) => typeof num === "number"))).sort(
    (a, b) => a - b,
  );
}
...
function normalizeTimelineFillItem(item, index) {
  ...
  const singleChapterCandidates = [item.chapter, item.chapterNumber, item.chapter_id];
  ...
  if (!chapters.length) {
    errors.push(
      `Item ${index} must include either "chapters" array, a single "chapter", or a "startChapter"/"endChapter" range.`,
    );
  }
...
export async function runTimelineFill({ profileOverride, quiet = true } = {}) {
  ...
  const timelineContext =
    typeof timelineMacroResult === "string"
      ? timelineMacroResult
      : JSON.stringify(timelineMacroResult, null, 2);

  const historyContext =
    typeof historyMacroResult === "string"
      ? historyMacroResult
    ...
    try {
      for (const task of tasks) {
        ...
        if (contiguous && chapters.length > 1) {
          ...
          if (chapterLimit > 0 && chapters.length > chapterLimit) {
            debug(
              `Timeline fill skipping query: exceeds ${chapterLimit}-chapter limit (${chapters.length} chapters requested)`,
            );
            continue;
          }
          try {
...
export async function queryChapters(startChapter, endChapter, query) {
  ...
  try {
    ...
    const allChaptersContext = chaptersData
      .map((chapterData) => {
        const chapterContent = chapterData.history
...
async function summarizeHistoryEntries(
  message_history,
  { targetMessageId, hideAfter = false, resummarizeChapterNumber = null } = {},
) {
  ...
  if (chunks.length === 1) {
    ...
    if (settings.add_chunk_summaries && targetMessageId !== undefined) {
      await runSlashCommand(
        `/comment at=${targetMessageId + 1} <details class="rmr-summary-chunks"><summary>Chunk Summaries</summary>${final_context}</details>`,
      );
    }
  } else {
    oopsToast("No visible chapter content! Skipping summary.");
...
async function generateChapterSummary(mes_id) {
  ...
  return await summarizeHistoryEntries(memory_history, {
    targetMessageId: mes_id,
    hideAfter: true,
  });
}
...
export async function summarizeChapter(messageOrId, options = {}) {
  ...
  const mes_id = typeof messageOrId === "number" ? messageOrId : Number(messageOrId.attr("mesid"));
  const chat = getContext().chat;
...
export async function resummarizeChapter(chapterNumber, options = {}) {
  ...
  const processedHistory = await Promise.all(
    rawHistory.map(async (message, offset) => {
      ...
      const mes_text = message.is_system
        ? original
        : getRegexedString(original, placement, options);
      return {
        ...message,
        mes: mes_text,
        index: absoluteIndex,
  ...
  const summary = await summarizeHistoryEntries(processedHistory, {
    targetMessageId: endIdx,
    hideAfter: false,
    resummarizeChapterNumber: chapterNumber,
  });
  if (!summary.length) {
    return "";
  }
...
export async function rememberEvent() {
  oopsToast(
    "Memory events are no longer saved to lorebooks. Chapters are now tracked in the timeline.",
  );
}
...
function validateArcItems(items) {
  ...
  return items
    .map((it, idx) => {
      ...
      return validId && title && summary
        ? { title, summary, justification, chapterEnd, _idx: idx }
        : null;
    })
    .filter(Boolean);
}
...
async function showArcPopup(arcs) {
  ...
  if (filteredArcs.length === 0) {
    await Popup.show.text(
      "Arc Analyzer",
      translate(
        "All arcs have been processed. Run Re-analyze for new arcs.",
        "rmr_arc_all_processed",
      ),
    );
    return;
  }
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (currentOverlay && document.body.contains(currentOverlay)) {
          ...
          const completedArcItem = currentOverlay.querySelector(
            `.rmr-arc-item[data-arc-end="${arc.chapterEnd}"]`,
          );
          if (completedArcItem) {
        ...
        const currentOverlay = arcSessionState.currentOverlay;
        if (currentOverlay && document.body.contains(currentOverlay)) {
          const errorArcItem = currentOverlay.querySelector(
            `.rmr-arc-item[data-arc-end="${arc.chapterEnd}"]`,
          );
          if (errorArcItem) {
  ...
  reanalyzeBtn.addEventListener("click", async () => {
    ...
    if (arcSessionState.summarizingArcEnd !== null) {
      toastr.warning(
        translate(
          "Please wait for current summarization to complete",
          "rmr_arc_wait_summarization",
        ),
        "Arc Analyzer",
      );
      return;
    ...
    try {
      ...
      if (filteredArcs.length === 0) {
        ...
        emptyEl.textContent = translate(
          "No valid arcs found. Try adjusting your prompts.",
          "rmr_arc_no_valid",
        );
...
export async function checkAutoSummarize() {
  ...
  debug(
    `Auto-summarize check: messagesSinceChapter=${messagesSinceChapter}, threshold=${N}, buffer=${X}, needed=${N + X}`,
  );
...
async function performAutoSummarize(lastChapterEnd, currentMsgId, N, X) {
  ...
  try {
    ...
    if (selectedEndpoint < rangeStart || selectedEndpoint > rangeEnd) {
      console.error(
        "Auto-summarize: Invalid endpoint selected:",
        selectedEndpoint,
        "valid range:",
        rangeStart,
        "-",
    ...
    debug(
      `Auto-summarize: Selected endpoint ${selectedEndpoint} (range: ${rangeStart}-${rangeEnd})`,
    );
