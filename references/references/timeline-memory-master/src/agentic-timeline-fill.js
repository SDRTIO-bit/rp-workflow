...
export async function recoverInterruptedSession() {
  ...
  if (savedState.startMessageIndex <= 0) {
    error(
      "Invalid startMessageIndex (0 or negative) - refusing to delete messages to prevent chat loss",
    );
  ...
  try {
    ...
    if (savedState.startMessageIndex <= chat.length) {
      ...
      error(
        `startMessageIndex (${savedState.startMessageIndex}) exceeds chat length (${chat.length}) - skipping message deletion`,
      );
    }
    ...
    if (savedState.hiddenMessageStart >= 0 && savedState.hiddenMessageEnd >= 0) {
      ...
      await executeSlashCommandsWithOptions(
        `/unhide ${savedState.hiddenMessageStart}-${savedState.hiddenMessageEnd}`,
      );
    }
...
export function registerAgenticTimelineTools() {
  ...
  context.ToolManager.registerFunctionTool({
    ...
    description:
      "Query a specific chapter from the timeline with a question. Returns information from that chapter based on your query.",
    stealth: false,
    parameters: {
      type: "object",
      properties: {
  ...
  const limitDescription =
    chapterLimit > 0 ? ` You can query a maximum of ${chapterLimit} chapters at a time.` : "";
    ...
    action: async (args) => {
      try {
        ...
        if (limit > 0 && chapterCount > limit) {
          log(
            `query_timeline_chapters rejected: ${chapterCount} chapters exceeds limit of ${limit}`,
          );
  ...
  context.ToolManager.registerFunctionTool({
    ...
    description:
      "List all entries in the character's assigned world info (lorebook). Returns the lorebook name, entry count, and details for each entry including uid, comment, keys, content, and settings. Use this to access stored world information and lore.",
    stealth: false,
    parameters: {
      type: "object",
      properties: {},
  ...
  context.ToolManager.registerFunctionTool({
    ...
    description:
      "Signal that you are done retrieving information from the timeline. Call this when you have gathered all necessary information.",
    stealth: true,
    parameters: {
      type: "object",
      properties: {
        final_information: {
          type: "string",
          description:
            "All the information you have been asked to provide. This will be saved and made available via {{timelineResponses}}.",
        },
      },
  ...
  context.ToolManager.registerFunctionTool({
    ...
    description:
      "Roll a die and get a random number between the specified minimum and maximum values (inclusive).",
    stealth: false,
    parameters: {
      type: "object",
      properties: {
...
export async function startAgenticTimelineFillSession() {
  ...
  if (context.chatMetadata?.lore_management_session?.active) {
    toastr.warning(
      "Cannot start timeline fill while lore management session is active",
      "Timeline Memory",
    );
    return;
  }
  ...
  // Save current state
  agenticTimelineFillState.savedProfileId =
    extension_settings.connectionManager?.selectedProfile || null;
  agenticTimelineFillState.savedProfileName = getProfileNameById(
    agenticTimelineFillState.savedProfileId,
  );
  ...
  try {
    ...
    if (firstVisibleIndex >= 0 && context.chat.length > 0) {
      ...
      log(
        `Hiding messages ${firstVisibleIndex} to ${agenticTimelineFillState.hiddenMessageEnd} for clean context`,
      );
      await executeSlashCommandsWithOptions(
        `/hide ${firstVisibleIndex}-${agenticTimelineFillState.hiddenMessageEnd}`,
      );
    }
...
function onGenerationEnded() {
  ...
  if (agenticTimelineFillState.retryCount > MAX_RETRIES) {
    ...
    toastr.error(
      "Agentic timeline fill aborted: AI failed to make valid tool calls",
      "Timeline Memory",
    );
  ...
  log(
    `Generation ended without tool call (retry ${agenticTimelineFillState.retryCount}/${MAX_RETRIES}), attempting retry...`,
  );
  ...
  setTimeout(async () => {
    ...
    for (let i = 0; i < SWIPE_RETRY_DELAYS.length; i++) {
      ...
      log(
        `Swipe attempt ${i + 1}/${SWIPE_RETRY_DELAYS.length}: waiting for swipe to be allowed...`,
      );
    ...
    toastr.error(
      "Agentic timeline fill aborted: Failed to retry after AI did not make a tool call",
      "Timeline Memory",
    );
    abortAgenticTimelineFillSession();
  }, 0);
...
async function triggerAgenticTimelineFill() {
  ...
  for (let attempt = 0; attempt <= INITIAL_TRIGGER_RETRY_DELAYS.length; attempt++) {
    ...
    try {
      ...
      log(
        `Initial trigger attempt ${attempt + 1} failed: ${err.message}. Retrying in ${delay}ms...`,
      );
      toastr.info(
        `Waiting for API to become ready... (attempt ${attempt + 1}/${INITIAL_TRIGGER_RETRY_DELAYS.length + 1})`,
        "Timeline Memory",
        { timeOut: delay },
      );
...
async function cleanupAgenticTimelineFillSession() {
  ...
  try {
    ...
    if (agenticTimelineFillState.startMessageIndex <= 0) {
      error(
        "Invalid startMessageIndex (0 or negative) - skipping message deletion in cleanup to prevent chat loss",
      );
      ...
      error(
        `startMessageIndex (${agenticTimelineFillState.startMessageIndex}) exceeds chat length (${chat.length}) - skipping message deletion`,
      );
    }

    // Unhide the messages that were hidden at session start
    if (
      agenticTimelineFillState.hiddenMessageStart >= 0 &&
      agenticTimelineFillState.hiddenMessageEnd >= 0
    ) {
      log(
        `Unhiding messages ${agenticTimelineFillState.hiddenMessageStart} to ${agenticTimelineFillState.hiddenMessageEnd}`,
      );
      await executeSlashCommandsWithOptions(
        `/unhide ${agenticTimelineFillState.hiddenMessageStart}-${agenticTimelineFillState.hiddenMessageEnd}`,
      );
    }
