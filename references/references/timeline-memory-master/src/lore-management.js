...
export function registerLoreTools() {
  ...
  context.ToolManager.registerFunctionTool({
    ...
    description:
      "List all entries in the character's assigned world info (lorebook). Returns the lorebook name, entry count, and details for each entry including uid, comment, keys, content, and settings. Use this to see what entries exist before editing them.",
    stealth: false,
    parameters: {
      type: "object",
      properties: {},
  ...
  context.ToolManager.registerFunctionTool({
    ...
    description:
      "Create, update, or delete an entry in the character's assigned world info (lorebook). For create/update, provide key (primary keywords array), content, and optionally keysecondary, comment, constant, and selective. For update/delete, uid is required.",
    stealth: false,
    parameters: {
      type: "object",
      properties: {
  ...
  context.ToolManager.registerFunctionTool({
    ...
    description:
      "Signal that you are done managing lorebooks. Call this when you have finished all lorebook edits.",
    stealth: true,
    parameters: {
      type: "object",
      properties: {},
...
export async function startLoreManagementSession() {
  ...
  if (context.chatMetadata?.agentic_timeline_fill_session?.active) {
    toastr.warning(
      "Cannot start lore management while timeline fill session is active",
      "Timeline Memory",
    );
    return;
  }
  ...
  // Save current state
  loreManagementState.savedProfileId =
  ...
  try {
    ...
    if (firstVisibleIndex >= 0 && context.chat.length > 0) {
      ...
      log(
        `Hiding messages ${firstVisibleIndex} to ${loreManagementState.hiddenMessageEnd} for clean context`,
      );
      await executeSlashCommandsWithOptions(
        `/hide ${firstVisibleIndex}-${loreManagementState.hiddenMessageEnd}`,
      );
    }
...
function onGenerationEnded() {
  ...
  log(
    `Generation ended without tool call (retry ${loreManagementState.retryCount}/${MAX_RETRIES}), attempting retry...`,
  );
  ...
  setTimeout(async () => {
    ...
    toastr.error(
      "Lore management aborted: Failed to retry after AI did not make a tool call",
      "Timeline Memory",
    );
    abortLoreManagementSession();
  }, 0);
...
async function cleanupLoreManagementSession() {
  ...
  try {
    ...
    if (loreManagementState.startMessageIndex <= 0) {
      ...
      error(
        `startMessageIndex (${loreManagementState.startMessageIndex}) exceeds chat length (${chat.length}) - skipping message deletion`,
      );
    }
    ...
    if (loreManagementState.hiddenMessageStart >= 0 && loreManagementState.hiddenMessageEnd >= 0) {
      log(
        `Unhiding messages ${loreManagementState.hiddenMessageStart} to ${loreManagementState.hiddenMessageEnd}`,
      );
      await executeSlashCommandsWithOptions(
        `/unhide ${loreManagementState.hiddenMessageStart}-${loreManagementState.hiddenMessageEnd}`,
      );
    }
