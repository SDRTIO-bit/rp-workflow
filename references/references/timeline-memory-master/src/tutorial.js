...
function createPopup() {
  ...
  if (isMobile) {
    ...
    const positionMobilePopup = (highlightedElement = null) => {
      ...
      const topValue = positionAtTop
        ? scrollY + margin
        : scrollY + viewportHeight - tutorialPopup.offsetHeight - margin;
  ...
  tutorialPopup.addEventListener(
    "touchstart",
    (e) => {
      e.stopPropagation();
    },
...
function onDrag(e) {
  ...
  const x = Math.max(
    0,
    Math.min(e.clientX - dragOffset.x, window.innerWidth - tutorialPopup.offsetWidth),
  );
  const y = Math.max(
    0,
    Math.min(e.clientY - dragOffset.y, window.innerHeight - tutorialPopup.offsetHeight),
  );
      ...
      if (isMobile) {
        ...
        setTimeout(() => {
          ...
          if (scrollContainer) {
            ...
            const elementOffsetInContainer =
