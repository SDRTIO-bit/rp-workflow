...
import {
  ...
  queryChapters,
  loadTimelineData,
  removeChapterFromTimeline,
  migrateTimelineData,
  getChapterSummary,
  resummarizeChapter,
  runTimelineFill,
  getTimelineFillResults,
  getTimelineEntries,
} from "./memories.js";
...
const profilesProvider = () => {
  ...
  return [
    ...
    ...extension_settings.connectionManager.profiles.map(
      (p) => new SlashCommandEnumValue(p.name, null, enumTypes.name),
    ),
  ];
};
...
async function removeReasoningBlocks(rangeInput) {
  ...
  try {
    ...
    if (!range) {
      toastr.error(
        "Invalid range format. Use a single ID (e.g., 5) or range (e.g., 1-10)",
        "Timeline Memory",
      );
      return "";
    }
    ...
    if (removedCount > 0) {
      ...
      toastr.success(
        `Removed reasoning blocks from ${removedCount} message${removedCount === 1 ? "" : "s"}`,
        "Timeline Memory",
      );
    } else {
    ...
    if (skippedCount > 0) {
      toastr.info(
        `Skipped ${skippedCount} user message${skippedCount === 1 ? "" : "s"}`,
        "Timeline Memory",
      );
    }
  ...
  for (let i = 0; i < chatLog.length; i++) {
    ...
    const isToolMessage =
      ...
      (message.is_system &&
        ...
        message.mes?.includes("Tool calls:")) ||
    ...
    if (isToolMessage) {
      ...
      if (i > 0) {
        ...
        if (previousMessage && previousMessage.is_user === false && !previousMessage.is_system) {
          const isSpecialSystemMessage =
            previousMessage.extra?.type === "narrator" || previousMessage.name === systemUserName;
...
function registerChapterQueryTool() {
  ...
  const limitDescription =
    chapterLimit > 0 ? ` Limited to a maximum of ${chapterLimit} chapters per query.` : "";
  context.ToolManager.registerFunctionTool({
    ...
    formatMessage: (args) => {
      const range =
        args.startChapter === args.endChapter
...
export function loadSlashCommands() {
    ...
    command.fromProps({
      ...
      namedArgumentList: [
        ...
        namedArg.fromProps({
          name: "title",
          description:
            "comment/title for the memory entry. only used when chapter end mode is `memory`",
          typeList: [arg_types.STRING],
          isRequired: false,
        }),
        namedArg.fromProps({
          name: "popup",
          description:
            'override the "popup memory" setting. only used when chapter end mode is `memory`',
      ...
      helpString:
        "Marks the message as a chapter endpoint and generates a summary from the previous endpoint. Defaults to the most recent message if no ID is provided.",
    }),
  );
  ...
  parser.addCommandObject(
    command.fromProps({
      ...
      helpString:
        "(Deprecated alias for /chapter-end) Marks the message as a chapter endpoint and generates a summary from the previous endpoint.",
    }),
  );
  ...
  parser.addCommandObject(
    command.fromProps({
        ...
        const executeFill = async () => {
          try {
            ...
            if (results.length === 0) {
              ...
              toastr.success(
                `Timeline fill stored ${successCount} result${successCount === 1 ? "" : "s"} in {{timelineResponses}}.`,
                "Timeline Memory",
              );
            } else {
              toastr.warning(
                `Timeline fill stored ${successCount} result${successCount === 1 ? "" : "s"}; ${errorCount} errored. Check console for details.`,
                "Timeline Memory",
              );
            }
          } catch (error) {
      ...
      helpString:
        "Generate timeline queries via the configured profile, execute them, and store results in {{timelineResponses}}. If agentic mode is enabled, starts an interactive session instead.",
    }),
  );
  ...
  parser.addCommandObject(
    command.fromProps({
      ...
      callback: async () => {
        try {
          const { abortAgenticTimelineFillSession, isAgenticTimelineFillActive } =
    ...
    command.fromProps({
      ...
      callback: (args, value) => {
        ...
        if (message) {
          ...
          if (removed) {
            toastr.success(
              `Chapter ${removed.summary.substring(0, 50)}... removed from timeline`,
              "Timeline Memory",
            );
          }
        }
      ...
      helpString:
        "Removes a chapter end marker and its timeline entry. Defaults to the most recent message if no ID is provided.",
    }),
  );
    ...
    command.fromProps({
      ...
      callback: (args, value) => {
        ...
        if (chapterNumber < 1 || chapterNumber > timeline.length) {
          toastr.error(
            `Chapter ${chapterNumber} is out of range (1-${timeline.length})`,
            "Timeline Memory",
          );
          return "";
        }
      ...
      helpString:
        "Force removes a chapter entry from the timeline by number, even if the chat marker is missing.",
    }),
  );
    ...
    command.fromProps({
      ...
      callback: (args, value) => {
        ...
        if (result.convertedFromPlaintext) {
          toastr.success(
            `Successfully converted ${result.migrated} timeline entries from plaintext to JSON format`,
            "Timeline Memory",
          );
        } else if (result.hadTimestamps) {
          toastr.success(
            `Successfully migrated ${result.migrated} timeline entries to the new format`,
            "Timeline Memory",
          );
        } else if (result.convertedSceneToChapter) {
          toastr.success(
            `Successfully converted scene markers to chapter markers in chat`,
            "Timeline Memory",
          );
        } else if (result.migrated === 0) {
          toastr.info(
            "No migration needed - timeline entries are already in the current format",
            "Timeline Memory",
          );
        } else {
      ...
      helpString:
        "Migrate old timeline entries to the new JSON format. Converts plaintext format including scene->chapter terminology to structured JSON and removes timestamps from legacy entries.",
    }),
  );
  ...
  parser.addCommandObject(
    command.fromProps({
      ...
      helpString:
        "Regenerate the summary for an existing chapter without altering its position in the timeline.",
    }),
  );
  ...
  parser.addCommandObject(
    command.fromProps({
      ...
      helpString:
        "Removes all tool call results and their invoking assistant messages from the chat history.",
