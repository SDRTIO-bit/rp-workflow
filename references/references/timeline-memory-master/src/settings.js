...
function reloadProfiles() {
  ...
  for (const profile of extension_settings.connectionManager.profiles) {
    profileSelect.append($("<option></option>").attr("value", profile.id).text(profile.name));
    ...
    if (arcProfileSelect?.length) {
      arcProfileSelect.append($("<option></option>").attr("value", profile.id).text(profile.name));
...
async function loadSettingsUI() {
  ...
  $("#rmr_memory_system_prompt").attr(
    "placeholder",
    defaultSettings.memory_system_prompt ||
      "System-level instructions for summarization (optional)",
  );
  $("#rmr_memory_prompt_template").attr("placeholder", defaultSettings.memory_prompt_template);
  $("#rmr_chapter_query_system_prompt").attr(
    "placeholder",
    defaultSettings.chapter_query_system_prompt ||
      "System-level instructions for chapter queries (optional)",
  );
  $("#rmr_chapter_query_prompt_template").attr(
    "placeholder",
    defaultSettings.chapter_query_prompt_template,
  );
  $("#rmr_arc_analyzer_system_prompt").attr(
    "placeholder",
    defaultSettings.arc_analyzer_system_prompt ||
      "System-level instructions for arc analysis (optional)",
  );
  $("#rmr_arc_analyzer_prompt_template").attr(
    "placeholder",
    defaultSettings.arc_analyzer_prompt_template,
  );
  $("#rmr_timeline_fill_system_prompt").attr(
    "placeholder",
    defaultSettings.timeline_fill_system_prompt ||
      "System-level instructions for timeline fill (optional)",
  );
  $("#rmr_timeline_fill_prompt_template").attr(
    "placeholder",
    defaultSettings.timeline_fill_prompt_template,
  );
  const mode_div = $(`#rmr_chapter_end_mode`);
  for (const end_mode in ChapterEndMode) {
    mode_div.append($("<option></option>").attr("value", end_mode).text(ChapterEndMode[end_mode]));
  ...
  $("#rmr_add_chunk_summaries")
    .prop("checked", settings.add_chunk_summaries)
    .on("click", toggleCheckboxSetting);
  $("#rmr_tools_enabled")
    .prop("checked", settings.tools_enabled)
    .on("click", async (e) => {
  ...
  $("#rmr_quick_reply_buttons_location")
    .val(settings.quick_reply_buttons_location || "send_form")
    .on("input", async () => {
  ...
  $("#rmr_loading_screen_enabled")
    .prop("checked", settings.loading_screen_enabled)
    .on("click", toggleCheckboxSetting);
  $("#rmr_shift_enter_hotkey_enabled")
    .prop("checked", settings.shift_enter_hotkey_enabled)
    .on("click", toggleCheckboxSetting);
  // handle dropdowns
  reloadProfiles();
  $("#rmr_profile").on("input", () => {
    ...
    const profileID = extension_settings.connectionManager?.profiles
      ? extension_settings.connectionManager.profiles.findIndex((it) => it.id == profile)
      : -1;
    if (profileID >= 0) {
  ...
  // Lore management settings
  $("#rmr_lore_management_enabled")
    .prop("checked", settings.lore_management_enabled)
    .on("click", toggleCheckboxSetting);
  $("#rmr_lore_management_prompt")
    .val(settings.lore_management_prompt)
    .on("change", handleStringValueChange);

  // Inject at depth settings
  $("#rmr_inject_enabled")
    .prop("checked", settings.inject_enabled)
    .on("click", async (e) => {
      toggleCheckboxSetting(e);
  ...
  // Agentic timeline fill settings
  $("#rmr_agentic_timeline_fill_enabled")
    .prop("checked", settings.agentic_timeline_fill_enabled)
    .on("click", toggleCheckboxSetting);
  $("#rmr_agentic_timeline_fill_prompt")
    .val(settings.agentic_timeline_fill_prompt)
    .on("change", handleStringValueChange);
  ...
  // Auto-summarize settings
  $("#rmr_auto_summarize_enabled")
    .prop("checked", settings.auto_summarize_enabled)
    .on("click", toggleCheckboxSetting);
  $("#rmr_auto_summarize_system_prompt").attr(
    "placeholder",
    defaultSettings.auto_summarize_system_prompt ||
      "System-level instructions for endpoint selection (optional)",
  );
  $("#rmr_auto_summarize_prompt_template").attr(
    "placeholder",
    defaultSettings.auto_summarize_prompt_template,
  );
  ...
  $("#rmr_query_chapter_limit")
    ...
    .on("change", async (e) => {
      ...
      const { isAgenticTimelineFillActive, registerAgenticTimelineTools } =
        await import("./agentic-timeline-fill.js");
...
function reloadPresetOptions(presetType) {
  ...
  presets.forEach((preset) => {
    select.append($("<option></option>").attr("value", preset.id).text(preset.name));
  });
...
function handleDeletePreset(presetType) {
  ...
  const confirmed = confirm(
    `Are you sure you want to delete the "${preset.name}" ${presetType} preset?`,
  );
...
${settings.keywords_prompt}`;
  ...
  if (settings.scene_query_prompt_template !== undefined) {
    ...
    settings.chapter_query_prompt_template = settings.scene_query_prompt_template
      .replace(/{{scene}}/gi, "{{chapter}}")
      .replace(/scenes/gi, "chapters");
  ...
  return new Promise((resolve) => {
    ...
    if (action) {
      ...
      const newName = prompt(
        `Enter a new name for the preset "${newPreset.name}":`,
        `${newPreset.name} - imported`,
      );
...
export async function importPreset(jsonData) {
  try {
    ...
    if (
      presetType !== "summarize" &&
      presetType !== "query" &&
      presetType !== "timeline_fill" &&
      presetType !== "arc"
    ) {
...
export async function importAllSettings(jsonData) {
  try {
    ...
    if (result.importSettings) {
      // Core settings
      if (importData.settings.is_enabled !== undefined)
        settings.is_enabled = importData.settings.is_enabled;
      if (importData.settings.tools_enabled !== undefined)
        settings.tools_enabled = importData.settings.tools_enabled;
      if (importData.settings.show_buttons !== undefined)
        settings.show_buttons = importData.settings.show_buttons;
      if (importData.settings.hide_chapter !== undefined)
        settings.hide_chapter = importData.settings.hide_chapter;
      if (importData.settings.add_chunk_summaries !== undefined)
        settings.add_chunk_summaries = importData.settings.add_chunk_summaries;
      if (importData.settings.chapter_end_mode !== undefined)
        settings.chapter_end_mode = importData.settings.chapter_end_mode;
      if (importData.settings.rate_limit !== undefined)
        settings.rate_limit = importData.settings.rate_limit;

      // Prompts
      if (importData.settings.memory_system_prompt !== undefined)
        settings.memory_system_prompt = importData.settings.memory_system_prompt;
      if (importData.settings.memory_prompt_template !== undefined)
        settings.memory_prompt_template = importData.settings.memory_prompt_template;
      if (importData.settings.chapter_query_system_prompt !== undefined)
        settings.chapter_query_system_prompt = importData.settings.chapter_query_system_prompt;
      if (importData.settings.chapter_query_prompt_template !== undefined)
        settings.chapter_query_prompt_template = importData.settings.chapter_query_prompt_template;
      if (importData.settings.timeline_fill_system_prompt !== undefined)
        settings.timeline_fill_system_prompt = importData.settings.timeline_fill_system_prompt;
      if (importData.settings.timeline_fill_prompt_template !== undefined)
        settings.timeline_fill_prompt_template = importData.settings.timeline_fill_prompt_template;
      if (importData.settings.arc_analyzer_system_prompt !== undefined)
        settings.arc_analyzer_system_prompt = importData.settings.arc_analyzer_system_prompt;
      if (importData.settings.arc_analyzer_prompt_template !== undefined)
        settings.arc_analyzer_prompt_template = importData.settings.arc_analyzer_prompt_template;
    }
    ...
    if (result.importSettings && result.importPresets) {
      ...
      if (importData.settings.current_timeline_fill_preset) {
        const preset = findPresetById(
          "timeline_fill",
          importData.settings.current_timeline_fill_preset,
        );
...
async function showMasterImportDialog(importData) {
  return new Promise((resolve) => {
    ...
    const totalPresets =
      presetCounts.summarize + presetCounts.query + presetCounts.timeline_fill + presetCounts.arc;

    const message =
      `Timeline Memory Configuration Import\n\n` +
      `This file contains:\n` +
...
export async function renderSummariesList() {
  ...
  if (!timeline || timeline.length === 0) {
    container.append(
      '<div class="rmr-summaries-empty">No chapters in current chat. End a chapter to create a summary.</div>',
    );
    return;
  }
...
async function openSummaryPopup(
  ...
  $(document)
    .off("keydown.summaryPopup")
    .on("keydown.summaryPopup", function (e) {
      if (e.key === "Escape") {
        closeSummaryPopup();
      }
    });
